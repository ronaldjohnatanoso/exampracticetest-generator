...
def make_banner(text, bg_color, fg_color=white, font_size=12, bold=True):
    """Create a colored banner paragraph."""
...
def md_to_pdf(md_path, pdf_path):
    doc = SimpleDocTemplate(
        ...
        topMargin=1.5 * cm,
        bottomMargin=1.5 * cm,
        title="AZ-305 Azure Solutions Architect — Cram Guide v2",
        author="Examify",
    )
    ...
    def add_h2(text):
        story.append(make_banner(text, LIGHT_BLUE, DARK_BLUE, 11, bold=True))
        story.append(Spacer(1, 4))
    ...
    while i < len(lines):
        ...
        if line.startswith('#### '):
            text = line.lstrip('# ').strip()
            story.append(Paragraph(f'<b>{text}</b>', h3))
            i += 1
            continue
        ...
        # Alert / tip blockquotes
        if '⚠️' in line or '💡' in line:
            text = re.sub(r'[⚠️💡]\s*', '', line.strip()).lstrip('>').strip()
            story.append(Paragraph(f'<b>{text}</b>', alert))
            i += 1
            continue
        ...
        if line.startswith('```'):
            ...
            code_para = Paragraph(f'<font face="Courier" size="7">{code_text}</font>', code)
    ...
    def add_page_number(canvas, doc):
        ...
        canvas.drawCentredString(PAGE_W / 2, 0.8 * cm,
                                  f"AZ-305 Cram Guide v2 — Page {page_num}")
        canvas.restoreState()
...
if __name__ == '__main__':
    md_to_pdf(
        '/home/ronald/exam-practice-test/study-guides/AZ-305-Cram-Guide.md',
        '/home/ronald/exam-practice-test/study-guides/AZ-305-Cram-Guide.pdf'
    )
